"""
Schémas de données pour le serveur MCP de traduction
"""
...
class TranslationRequest(BaseModel):
    ...
    target_language: str = Field(...,
                                 description="Code langue cible (ex: 'fr', 'en', 'es')")
    user_id: str = Field(...,
...
class TranslationStatusResponse(BaseModel):
    ...
    original_status: Optional[str] = Field(
        None, description="Statut original de l'API Azure")
    progress: Optional[str] = Field(
        None, description="Information de progression")
    created_at: Optional[str] = Field(None, description="Date de création")
    last_updated: Optional[str] = Field(
        None, description="Dernière mise à jour")
    error: Optional[str] = Field(None, description="Message d'erreur si échec")
    summary: Optional[Dict[str, int]] = Field(
        None, description="Résumé des statistiques")
...
class OneDriveUploadResult(BaseModel):
    ...
    onedrive_url: Optional[str] = Field(
        None, description="URL OneDrive du fichier")
    error: Optional[str] = Field(None, description="Message d'erreur")


class MCPToolResponse(BaseModel):
    """Réponse standard d'un outil MCP"""
    success: bool = Field(..., description="Succès de l'opération")
    message: str = Field(..., description="Message descriptif")
    data: Optional[Dict[str, Any]] = Field(
        None, description="Données additionnelles")


class SupportedLanguages:
    """Langues supportées par Azure Translator"""
